...
const fetchBitMex = async (since = 0) => {
  try {
    ...
    while (since < now.getTime()) {
      ...
      let partial = await bitmex.fetchOHLCV("BTC/USD", "1m", null, null, {
        startTime: since,
        count: 750,
      });
      ...
      console.log(
        ...
        since,
        lastTs,
        new Date(since).getTime() === new Date(lastTs).getTime()
      );
      if (
        since.getTime() === new Date(partial[partial.length - 1][0]).getTime()
      )
        break;
